...
#
# $Id: PacketGen.pm 4251 2008-07-09 17:06:56Z jnaous $
#
...
#                             should no longer be needed for NF2.
#
###############################################################
...
sub nf_add_port_rule {
  ...
  unless (($port > 0) and ($port <= $NF2_MAX_PORTS)) {
    confess "nf_add_port_rule(): port must be 1..$NF2_MAX_PORTS (saw \"$port\")"
  }
...
# See NF2/doc/file_formats.txt for format
# Params: none
...
variable includes the NF2/lib/Perl5 directory and then add the
...
actual physical ethernet port on the NF2 board (it has 4 ports).
