...
my $fields_with_vlan_2 =
  {NFUtils::SimplePacket::TRANSP_DST() => [0x1111],
   ...
   NFUtils::SimplePacket::SRC_PORT() => [0, 2, 4, 6],
   NFUtils::SimplePacket::IP_TOS() => [0],
   NFUtils::SimplePacket::VLAN_TAG() => [0xe123],
   NFUtils::SimplePacket::PKT_TYPE() => [NFUtils::SimplePacket::PKT_TYPE_UDP]};
...
my $nf2_action_flag = 0x0006;  # OFPAT_SET_VLAN_VID and OFPAT_SET_VLAN_PCP
my $set_vlan_vid = 0x123;
my $set_vlan_pcp = 0x7;
my $set_dl_src = 0;
my $set_dl_dst = 0;
my $set_nw_src = 0;
...
# Send pkt w/o VLAN tag, expect pkt w/ VLAN tag
...
$delay_us += NFOpenFlowTester::runRandomSim(\&NFOpenFlowTester::expDifferentPkt,
					    ...
					    NFOpenFlowTester::FLOW_ENTRY_FIELDS() => $fields_with_vlan_1,
					    NFOpenFlowTester::PKT_FIELDS() => $fields_with_vlan_1,
                                            NFOpenFlowTester::EXP_PKT_FIELDS() => $fields_with_vlan_2,
