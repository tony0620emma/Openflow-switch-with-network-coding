...
#
# $Id: GenericByteObject.pm 3703 2008-05-08 00:55:59Z jnaous $
...
sub int_to_bytes {
  ...
  if($value > 0 && ByteArrayUtils::log2($value) > $width*8) {
    die "Error: $value is too large. It has to fit in $width bytes\n";
  }
...
sub bytes_to_int {
  ...
  die "Error: Too many elements in the list of bytes to convert to int. Should be $width, found @byte_list.\n" unless ((scalar @byte_list) == $width);
...
sub mac_to_bytes {
  ...
  die "MAC address should be specified as xx:xx:xx:xx:xx:xx found $mac.\n" unless ($mac =~ m/^..:..:..:..:..:..$/);
...
#  print "setting $field at $pos with width $width to $val in bytes:";
